...
def rename(name):
    ...
    return patient_id[0] + "_" + image_side + "_" + image_type
...
def crop_img(img):
    ...
    if cropped_img.shape[0] == cropped_img.shape[1]:
        ...
        cropped_img = cv2.resize(cropped_img.astype('float32'), dsize=(tile_size, tile_size), interpolation=cv2.INTER_CUBIC)
